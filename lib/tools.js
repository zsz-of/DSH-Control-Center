 ...
 * - `ask` 每次询问：走平台的审批通道；若会话权限预设是 `never`（danger-full-access），先按用户的这个
 *   选择把本会话策略拨回 `ask`，拨不动才回报「预设关闭了审批」并指路；
 * - `allow` 始终同意：直接放行；
 * - `deny-once` 禁止一次：拒掉这一次后自动回到 `ask`；
 * - `deny-always` 禁止且不再询问：一律拒绝。
 *
 * 那个「预设关闭了审批」的回报是必要的：平台在 approval=never 下会在弹窗前直接判 rejected，理由却写成
 * `the user rejected tool …`，看着像用户拒绝、实际用户根本没被问过。
 *
...
export function registerTools(ctx, runtime) {
  ...
  disposers.push(
    ...
    ctx.on('tools/pre-execute', async (exec, next) => {
      ...
      const mode = (await readSettings()).rules.approval

      if (mode === 'allow') {
        // 用户明确选了「始终同意」：直接放行，不惊动平台的审批通道。
        return next()
      }
      if (mode === 'deny-always') {
        return {
          kind: 'deny',
          reason: `控制中心：规则写入审批被设为「禁止且不再询问」，${exec.name} 不会执行。想让我改规则，请在「规则」页把审批改成「每次询问」或「始终同意」。`,
        }
      }
      if (mode === 'deny-once') {
        // 一次性档位：拒掉这次就自动回到「每次询问」，免得用户忘了自己设过这一档。
        await writeSettings({ rules: { approval: 'ask' } }).catch(() => {})
        return {
          kind: 'deny',
          reason: `控制中心：规则写入审批是「禁止一次」，所以这次没有执行 ${exec.name}；审批已自动回到「每次询问」，下次会先问你。`,
        }
      }

      // 「每次询问」：会话权限预设可能把审批整个关掉（danger-full-access → approval=never）。
      // 平台的 approval 服务此时会在弹窗前直接判 rejected，理由却写成
      // `the user rejected tool "…"`，看着像用户拒绝、实际用户根本没被问过。
      // 用户既然在设置里选了「每次询问」，就先把本会话策略拨回 ask（这与权限预设的 GUI 走同一条
      // `setPolicy` 通路）；拨不动才解释真正的原因。（ctx.get 可能不存在，必须可选调用。）
      ...
      if (policy === 'never') {
        if (exec.agent === undefined || typeof approval?.setPolicy !== 'function') {
          return {
            kind: 'deny',
            reason: `控制中心：当前会话的权限预设关闭了审批（approval=never，例如 danger-full-access），${exec.name} 会被平台自动拒绝，所以不会弹确认框。想让我改规则，请把权限预设切成 workspace-write；也可以在「规则」页把审批设为「始终同意」。`,
          }
        }
        try {
          ...
          return {
            kind: 'deny',
            reason: `控制中心：这次想替你把本会话审批打开（${failure?.message ?? failure}），但没成功，所以不会弹确认框。请把权限预设切成 workspace-write；也可以在「规则」页把审批设为「始终同意」。`,
          }
        }
      }
      return {
