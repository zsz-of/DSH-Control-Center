 ...
 * 为 `never` 时这类调用会被直接拒绝——那是用户自己关掉的，不是本插件吞掉。闸门会先自检这个策略，
 * 为 `never` 时直接回报「预设关闭了审批」并指路，免得平台那句 `the user rejected tool …` 看着像用户拒绝。
 *
...
export function registerTools(ctx, runtime) {
  ...
  disposers.push(
    ctx.on('tools/pre-execute', (exec, next) => {
      ...
      // 会话权限预设可能把审批整个关掉（danger-full-access → approval=never）。
      // 平台的 approval 服务此时会在弹窗前直接判 rejected，理由却写成
      // `the user rejected tool "…"`，看着像用户拒绝、实际用户根本没被问过。
      // 先自检策略，把真正的原因说出来。（ctx.get 可能不存在，必须可选调用。）
      const policy = ctx.get?.('approval')?.effectivePolicy?.(exec.agent?.session)
      if (policy === 'never') {
        return {
          kind: 'deny',
          reason: `控制中心：当前会话的权限预设关闭了审批（approval=never，例如 danger-full-access），${exec.name} 会被平台自动拒绝，所以不会弹确认框。想让我改规则，请把权限预设切成 workspace-write；也可以你自己在控制中心界面里改。`,
        }
      }
      return {
        kind: 'ask',
