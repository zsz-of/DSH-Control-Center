    ...
    /**
     * 「AI 改规则时的审批」四档，顺序即下拉顺序（与 host 侧 `settings.js` 的 `RULE_APPROVALS` 一一对应）。
     *
    ...
    /** 选中某一档时的解释：说清「接下来会发生什么」，尤其是两个否定档位的差别。 */
    const RULE_APPROVAL_HINTS = {
      ask: 'AI 每次要改规则都先问你（若本会话的权限预设关掉了审批，会先替你把它打开）。',
      allow: 'AI 可以直接改规则，不再问你。',
      'deny-once': '只拒绝下一次改规则，之后自动回到「每次询问」。',
