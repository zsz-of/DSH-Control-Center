...
/**
 * 「AI 改规则」的四档审批（对应 `settings.rules.approval`），顺序即界面顺序：
 *
 * - `ask`：每次询问（默认；必要时会先把本会话的审批打开，否则平台会替你拒绝）
 * - `allow`：始终同意，直接放行
 * - `deny-once`：禁止一次，拒掉下一次改规则后自动回到 `ask`
 * - `deny-always`：禁止且不再询问，一律拒绝
 */
export const RULE_APPROVALS = ['ask', 'allow', 'deny-once', 'deny-always']
