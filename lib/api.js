 ...
 *
 * 三个曾经的坑，都在这里补掉：
 * 1. **目标不存在时什么都不发生**：`~/.dsh/memory`、`~/.dsh/mcp.json` 在用户还没建过时并不存在，
 *    而 `explorer.exe` 对不存在的路径既不报错也不开窗——界面看起来就是「按钮坏了」。
 *    这里先把目录建出来（定位文件时建父目录），确保落点真的能打开。
 * 2. **失败无处可报**：`spawn` 的 `error` 事件没人监听会变成未捕获异常；现在等 spawn 结果再返回，
 *    界面因此能显示原因，而不是静默什么都不做。
 * 3. **Windows 上定位文件要 `/select,`**：直接传文件路径会被 explorer 当成「打开这个文件」。
 *
 * @param target - `{ dir }` 打开目录，或 `{ file }` 定位文件。
 * @param launch - 启动器（默认 `node:child_process.spawn`；测试注入假实现，免得真开窗）。
 * @returns `{ target, kind }`——实际打开的路径与方式。
 */
export async function reveal(target, launch = spawn) {
  const select = typeof target?.file === 'string' && target.file !== ''
  const path = select ? target.file : typeof target?.dir === 'string' && target.dir !== '' ? target.dir : DATA_ROOT
  const dir = select ? dirname(path) : path
  try {
    await mkdir(dir, { recursive: true })
  } catch (error) {
    throw new Error(`无法创建目录 ${dir}：${error?.message ?? String(error)}`)
  }
  ...
  const [command, args] =
    process.platform === 'win32'
      ...
      : process.platform === 'darwin'
        ...
        : ['xdg-open', [select ? dir : path]]

  await new Promise((resolve, reject) => {
    let child
    try {
      child = launch(command, args, { detached: true, stdio: 'ignore', windowsHide: true })
    } catch (error) {
...
/** 处理一次 API 请求。 */
async function handle(req, res, runtime, url) {
  /**
   * 请求里带的工作区。
   *
  ...
  try {
    if (section === 'rule') {
      ...
      // 而不是静默什么都不发生。`file` 优先于 `path`，两者都可以不给（默认数据根目录）。
      const file = payload.file === undefined ? undefined : String(payload.file)
      const dir = payload.path === undefined ? undefined : String(payload.path)
      extra = await reveal(file === undefined ? { dir } : { file })
    } else {
 ...
 * @returns disposer。
 */
export function registerApi(ctx, runtime) {
  ...
  return ctx.webServer.register({
    ...
    handler: (req, res) => {
      ...
      handle(req, res, runtime, url).catch((error) => {
