...
| 页面 | 能力 |
|---|---|
| **规则** | **全局规则**（`~/.dsh/rules`）、新建 / 编辑 / 删除 / 启停、**强加载**与**普通模式**两种加载方式、搜索、注入占用统计、打开目录、**AI 改规则时的审批档位**（每次询问 / 始终允许 / 禁止且不再询问）与已豁免工作区的清除 |
...
不需要手动挑文件夹。切到这个页签时，平台底部那条输入框和左右两条宽度拖拽手柄会一起收起来——
这个页面不发消息，留着它们只会误点。
...
AI 调 `control_center_rule_write` / `control_center_rule_delete` 时，插件会在对话里弹一个**四按钮的确认框**
（卡片顶部是一条淡黄色醒目条，写明「控制中心 · 需要你确认」，一眼能认出这是控制中心的审批而不是随口一问）：

| 弹框上的按钮 | 会发生什么 |
|---|---|
| **本轮对话中始终允许** | 这次放行，并且这一轮对话里不再问（重启 DSH 桌面后重新开始询问） |
| **此工作区内始终允许** | 这次放行，并且这个工作区以后都不再问（可在规则页清除） |
| **禁止一次** | 只拒绝这一次，下次照样问你 |
| **禁止且不再询问** | 拒绝，并把规则页的审批档位改成「禁止且不再询问」 |

档位由用户在**规则页**自己选，存在本插件设置里（`rules.approval`），两处规则视图（控制中心的规则页、
会话里的项目规则视图）都能改：

| 档位 | 行为 |
|---|---|
| **每次询问**（默认） | 每次都弹上面那个四按钮的框 |
| **始终允许** | 直接放行，不再问 |
| **禁止且不再询问** | 一律拒绝 AI 改规则，直到你改回其它档位 |

「禁止一次」不是档位，只是那个弹框上的一个按钮。规则页还会列出**已豁免的工作区**（弹框上点过「此工作区
内始终允许」的那些）并给一个「清除」按钮，点掉之后重新恢复询问。

弹框走的是 DSH 的「提问」通道（与 `ask_user_question` 同一张卡片）。平台自带的那张审批卡片只有
「拒绝 / 允许一次」两个按钮，插件改不了它，所以规则写入改成由插件自己问、自己记；只有这条通道不可用时
（例如在子 agent 里）才退回平台审批通道——那时若会话权限预设把审批关了（`danger-full-access` →
`approval: never`），会先替你把本会话策略拨回 `ask`，拨不动才回报「预设关闭了审批」并指路。
