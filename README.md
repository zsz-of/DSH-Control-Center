...
| 页面 | 能力 |
|---|---|
| **规则** | **全局规则**（`~/.dsh/rules`）、新建 / 编辑 / 删除 / 启停、**强加载**与**普通模式**两种加载方式、搜索、注入占用统计、打开目录、**AI 改规则时的审批档位**（每次询问 / 始终同意 / 禁止一次 / 禁止且不再询问） |
...
这两个工具经 `tools/pre-execute` 交给平台的审批通道，用户在对话里同意（allowed-once）才落盘。

审批档位由用户在**规则页**自己选，存在本插件设置里（`rules.approval`），两处规则视图（控制中心的规则页、
会话里的项目规则视图）都能改：

| 档位 | 行为 |
|---|---|
| **每次询问**（默认） | 每次都走平台审批弹框；若当前会话的权限预设把审批关了（`danger-full-access` → `approval: never`），先替你把本会话策略拨回 `ask`，拨不动才回报「预设关闭了审批」并指路 |
| **始终同意** | 直接放行，不再问 |
| **禁止一次** | 拒掉下一次改规则，之后自动回到「每次询问」 |
| **禁止且不再询问** | 一律拒绝 AI 改规则，直到你改回其它档位 |

「每次询问」不动平台自己的权限预设（设置 → 权限），只在本会话内把审批打开——平台在 `approval: never`
下会在弹窗前直接判 rejected，理由却写成 `the user rejected tool …`，看着像用户拒绝、实际用户根本没被问过。
